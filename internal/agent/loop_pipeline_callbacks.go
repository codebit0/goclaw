...
func (l *Loop) makeCallLLM(req *RunRequest, emitRun func(AgentEvent)) func(ctx context.Context, state *pipeline.RunState, chatReq providers.ChatRequest) (*providers.ChatResponse, error) {
	return func(ctx context.Context, state *pipeline.RunState, chatReq providers.ChatRequest) (*providers.ChatResponse, error) {
		...
		chatReq.Options[providers.OptAgentKey] = l.id
		...
		chatReq.Options[providers.OptTeamWorkspace] = tools.ToolTeamWorkspaceFromCtx(ctx)
		...
		if l.bridgeTraceReg != nil && l.traceCollector != nil && spanID != uuid.Nil {
		...
		if req.Stream {
			...
			resp, err = provider.ChatStream(ctx, chatReq, func(chunk providers.StreamChunk) {
				...
				if chunk.ToolName != "" && chunk.ToolCallID != "" {
					...
					now := time.Now().UTC()
					toolSpanID := l.emitToolSpanStart(ctx, now, chunk.ToolName, chunk.ToolCallID, string(inputJSON))
					cliToolSpans[chunk.ToolCallID] = cliToolSpan{spanID: toolSpanID, start: now, name: chunk.ToolName}
					emitRun(AgentEvent{
				...
				if chunk.ToolResult != "" && chunk.ToolCallID != "" {
					if span, ok := cliToolSpans[chunk.ToolCallID]; ok {
						toolRes := &tools.Result{ForLLM: chunk.ToolResult, IsError: chunk.ToolIsError}
						l.emitToolSpanEnd(ctx, span.spanID, span.start, toolRes)
